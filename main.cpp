...
int main(int argc, char **argv)
{
   ...
   runManager->SetUserInitialization(new XPDetectorConstruction());

   // Physics list
   //auto physicsList = new XPPhysicsList();
   auto physicsList = new QGSP_BERT_HP();
   runManager->SetUserInitialization(physicsList);

   // Primary generator action and User action intialization
   runManager->SetUserInitialization(new XPActionInitialization());
